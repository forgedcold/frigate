...
class RecordingMaintainer(threading.Thread):
    ...
    async def move_files(self) -> None:
        ...
        for cache in cache_files:
            ...
            is_playback = "~playback" in camera
            if is_playback:
                camera = camera.replace("~playback", "")
        ...
        for camera in grouped_recordings.keys():
            ...
            if processed_segment_count > keep_count:
                ...
                to_remove = non_playback[:-keep_count]
            ...
            unprocessed_segment_count = (
                len(non_playback) - processed_segment_count
            )
            ...
            orphaned_playback = [r for r in playback if r["start_time"] < oldest_full]
        ...
        for camera, recordings in grouped_recordings.items():
            non_playback = [r for r in recordings if not r.get("is_playback", False)]
            oldest_start = (
                non_playback[0]["start_time"].timestamp()
                if non_playback
                else recordings[0]["start_time"].timestamp()
            )

            # clear out all the object recording info for old frames
            while (
                len(self.object_recordings_info[camera]) > 0
                and self.object_recordings_info[camera][0][0] < oldest_start
            ):
            ...
            while (
                ...
                and self.audio_recordings_info[camera][0][0] < oldest_start
            ):
            ...
            reviews: ReviewSegment = (
                ...
                .where(
                    ...
                    | (ReviewSegment.end_time >= oldest_start),
                )
    ...
    async def validate_and_move_segment(
        ...
        is_playback: bool = recording.get("is_playback", False)
        ...
        try:
            if not os.path.exists(file_path):
                ...
                return {
                    ...
                    Recordings.motion.name: (
                        0 if is_playback else segment_info.motion_count
                    ),
                    # TODO: update this to store list of active objects at some point
                    Recordings.objects.name: (
                        0 if is_playback else segment_info.active_object_count
                    ),
                    Recordings.regions.name: (
                        0 if is_playback else segment_info.region_count
                    ),
                    Recordings.dBFS.name: (
                        0 if is_playback else segment_info.average_dBFS
                    ),
